...
public class GeminiClient {
  ...
  @Value("${gemini-client-api-url:https://generativelanguage.googleapis.com/v1beta/models/gemini-2"
      + ".0-flash:generateContent}")
  private String apiUrl;

  private final OkHttpClient client =
      new OkHttpClient.Builder().connectTimeout(60, TimeUnit.SECONDS).readTimeout(60, TimeUnit.SECONDS)
  ...
  public String generateContent(String prompt) throws IOException {
    ...
    Request request =
        new Request.Builder().url(apiUrl + "?key=" + apiKey).post(body).addHeader("Content-Type", "application/json")
            .build();

    try (Response response = client.newCall(request).execute()) {
      if (!response.isSuccessful()) {
        throw new IOException("Unexpected code " + response + " with body: " + (response.body() != null ?
